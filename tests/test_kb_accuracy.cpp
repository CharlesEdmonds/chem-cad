...
TEST_CASE("kirkwood-buff predictions sit inside the accuracy envelope") {
  ...
  for (const sol::SolubilityAnchor& anchor : sol::anchors()) {
    ...
    std::printf("%-34s measured %10.5g  KB %10.5g  log10 %+6.2f  logP %6.2f  Tm %6.1f  V2 %6.1f\n",
                row.label.c_str(), row.measured, row.kb, row.log10Err, solute.logP,
                solute.meltingPoint, solute.molarVolume);
  }
  ...
  //
  //   median |log10 err| ~ 0.63  (a factor of ~4)
  //   95% of pairs       <= 1.5  (a factor of ~30)
  //   75% of pairs       <= 1.0  (an order of magnitude)
  //
  // Adding the Yalkowsky GSE for the aqueous share moved the water column
  // from a median of 1.18 log to 0.38 log; what remains, largest first:
  ...
  //      is under-predicted by ~1.4 log because chloroform's C-H...O=C
  ...
  //
  // Anchored pairs bypass all of this in the shipped app: predict() returns
  // the measured value exactly. These bounds describe EXTRAPOLATION quality.
  CHECK(within(1.5) >= 0.95);
  CHECK(within(1.0) >= 0.75);
  CHECK(median <= 0.75);
}
